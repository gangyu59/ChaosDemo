...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let intervalId = null;
    let currentTouchHandler = null;  // track Koch interaction handler for cleanup

    function clearCanvasAndStop(removeKochListener = true) {
        stopDoublePendulum(); // 调用来自 doublePendulum.js 的停止函数
        if (intervalId !== null) {
            clearInterval(intervalId);
            intervalId = null;
        }
        if (removeKochListener && currentTouchHandler) {
            canvas.removeEventListener('pointerdown', currentTouchHandler);
    ...
    document.getElementById('showKoch').addEventListener('click', function() {
        clearCanvasAndStop(false);
        drawKoch(canvas, ctx, clearCanvasAndStop);  // Draw Koch curve
        // Remove existing interaction listener to prevent duplicates
        if (currentTouchHandler) {
            canvas.removeEventListener('pointerdown', currentTouchHandler);
        }
        // Use pointer events to support touch + mouse click
        currentTouchHandler = handleTouch(canvas, ctx, clearCanvasAndStop);
        canvas.addEventListener('pointerdown', currentTouchHandler);
    });

    document.getElementById('showDoublePendulum').addEventListener('click', function() {
        clearCanvasAndStop();
        drawDoublePendulum(canvas, ctx, clearCanvasAndStop);
    });
